...
graph TB
    ...
    subgraph API ["API (Node.js / Fastify)"]
        ...
        Instrumentation[instrumentation.ts - OTel SDK]
        Config[config - Env Validation]
        DI[deps - Awilix DI Container]
        TracedProxy[tracing/traced-proxy - DI Instrumentation]
    ...
    subgraph GameGen ["Game Generator (TypeScript Library)"]
        Tracing[tracing - @traced & withSpan]
    ...
    Server --> GameRoutes
    Server --> Health
    Instrumentation -.->|auto-instrument| Server
    DI --> TracedProxy
    DI --> Sources
    TracedProxy -.->|wrap| Sources
    Sources --> Quotes
...
│   │   │   │   ├── tracing/      # tracedProxy for DI service instrumentation
...
│   │   │   │   └── instrumentation.ts  # OpenTelemetry SDK (loaded via --import)
│   │   │   └── tests/helpers/    # Test container factory & mocks
│   │   └── game-generator/       # Puzzle generation library
│   │       └── src/
│   │           ├── cipher/       # Keyword substitution cipher (@traced methods)
│   │           ├── data/         # Keywords list, quotes schema
│   │           ├── difficulty/   # 7-factor difficulty scoring
│   │           ├── hints/        # Frequency-based hint selection (withSpan)
│   │           ├── quotes/       # QuoteSource abstract class
│   │           ├── tracing.ts    # @traced decorator & withSpan HOF
│   │           ├── random.ts     # Seeded RNG (XorShift128+)
│   │           └── validation.ts # Timing-safe solution comparison (withSpan)
...
├── docs/
│   ├── design-plans/             # Dated design documents
│   └── test-plans/               # Human verification test plans
...
**Purpose**: Fastify 5 REST API serving cryptoquip puzzles with OpenAPI documentation and distributed tracing
...
| `src/instrumentation.ts` | OpenTelemetry SDK init (ESM loader hook for HTTP, Fastify, Pino, Undici) | 645 |
...
| `src/deps/singleton.ts` | Singleton DI container (wraps quoteSource/keywordSource with tracedProxy) | 500 |
...
| `src/tracing/traced-proxy.ts` | ES6 Proxy wrapper creating spans for DI service method calls | 703 |
...
| `src/sources/json-quote-source.ts` | JsonQuoteSource — file-based, lazy-loaded, TypeBox-validated | 419 |
| `src/sources/static-keyword-source.ts` | StaticKeywordSource — wraps KEYWORDS constant for DI | 80 |

**Key exports**: `buildServer()`, `start()`, `tracedProxy()`, route plugins

**Dependencies**: `@unquote/game-generator` (workspace), Fastify ecosystem, Awilix, Sqids, Luxon, OpenTelemetry SDK + instrumentations
...
**Purpose**: Pure library transforming quotes into cryptoquip puzzles with deterministic daily generation and OpenTelemetry span instrumentation

**Entry point**: `src/index.ts`

| File | Purpose | Tokens |
|------|---------|--------|
| `src/index.ts` | Public API surface (types, contracts, implementations, tracing utilities) | 135 |
| `src/tracing.ts` | `@traced` class method decorator, `withSpan` HOF, shared `executeInSpan` core | 685 |
...
| `src/validation.ts` | Timing-safe solution comparison (NFC + case + whitespace normalization, withSpan) | 325 |
| `src/cipher/keyword-cipher.ts` | Keyword substitution cipher (@traced methods, withSpan on internals) | 1342 |
| `src/cipher/types.ts` | GameGenerator and KeywordSource interfaces | 255 |
...
| `src/hints/generator.ts` | Reveals uncommon cipher letters as hints (withSpan) | 434 |
| `src/quotes/types.ts` | QuoteSource abstract class (getAllQuotes abstract, getQuote/getRandomQuote/ensureLoaded concrete) | 381 |
| `src/quotes/in-memory-source.ts` | InMemoryQuoteSource — in-memory array-backed source (test/dev usage) | 99 |
| `src/testing.ts` | Re-exports InMemoryQuoteSource for test consumers | 51 |

**Key exports**: `KeywordCipherGenerator`, `InMemoryQuoteSource`, `QuoteSource`, `KeywordSource`, `KEYWORDS`, `validateSolution`, `traced`, `withSpan`, domain types

**Dependencies**: Luxon (dates), TypeBox (validation), OpenTelemetry API (tracing — no-op when SDK not registered)
...
| `internal/app/update.go` | Message handler: keys, mouse, API responses, timer ticks | 2270 |
| `internal/app/view.go` | Render: header, hints, grid, status bar, help | 1084 |
| `internal/app/grid.go` | Word-wrapped puzzle grid with highlight precedence | 1055 |
| `internal/app/commands.go` | Async commands: fetch puzzle, check solution, load/save session | 805 |
| `internal/app/messages.go` | Message types for Bubble Tea | 164 |
| `internal/puzzle/cells.go` | Cell model + navigation (next/prev/unfilled letter cells) | 728 |
| `internal/puzzle/solution.go` | Solution assembly, input propagation, completeness check | 462 |
| `internal/puzzle/difficulty.go` | Score→text mapping (Easy/Medium/Hard/Expert) | 74 |
| `internal/storage/session.go` | XDG state persistence (atomic writes, path traversal protection via os.OpenRoot) | 975 |
| `internal/ui/styles.go` | Lip Gloss color scheme and cell styles | 648 |
...
**Dependencies**: Bubble Tea 1.3, Lip Gloss 1.1, BubbleZone 1.0, XDG 0.5
...
| File | Purpose | Tokens |
|------|---------|--------|
| `api-docker-pr.yml` | PR: validate Docker build (buildx, GHA cache) | 731 |
| `api-release.yml` | Release: CI checks, semantic tag, native multi-arch Docker build + GHCR push | 2160 |
| `tui-pr.yml` | PR: lint, test, GoReleaser snapshot build with artifact upload + PR comment | 2129 |
| `tui-release.yml` | Release: semantic version tag + GoReleaser GitHub Release | 1313 |
...
| File | Purpose | Tokens |
|------|---------|--------|
| `CLAUDE.md` | Developer instructions (tech stack, commands, architecture, observability) | 1386 |
...
| `README.md` | Project overview with install/play instructions | 748 |
...
sequenceDiagram
    ...
    Note over API: Auto-instrumented HTTP + Fastify spans
    API->>Generator: generateDailyPuzzle(today)
    Note over Generator: @traced creates span
    Generator->>Generator: hashString("2026-02-15") to seed
    Generator->>QuoteSource: getRandomQuote(seed)
    Note over QuoteSource: tracedProxy creates span
    QuoteSource-->>Generator: Quote
    Generator->>Generator: selectKeyword(seed) to cipher alphabet
    Note over Generator: withSpan on each cipher step
...
sequenceDiagram
    ...
    Note over API: withSpan creates validateSolution span
...
### Tracing Span Tree (GET /game/:date)

```mermaid
graph TB
    A["HTTP GET /game/:date<br/>(auto: @opentelemetry/instrumentation-http)"]
    B["request handler - /game/:date<br/>(auto: @opentelemetry/instrumentation-fastify)"]
    C["QuoteSource.getRandomQuote<br/>(tracedProxy, attr: service.implementation=JsonQuoteSource)"]
    D["KeywordSource.getKeywords<br/>(tracedProxy, attr: service.implementation=StaticKeywordSource)"]
    E["KeywordCipherGenerator.generateDailyPuzzle<br/>(@traced decorator)"]
    F["KeywordCipherGenerator.generatePuzzle<br/>(@traced decorator)"]
    G["buildCipherAlphabet<br/>(withSpan)"]
    H["buildCipherMapping<br/>(withSpan)"]
    I["eliminateSelfMappings<br/>(withSpan)"]
    J["encryptText<br/>(withSpan)"]
    K["generateHints<br/>(withSpan)"]
...
- **API**: Awilix with singleton + request scopes; DI services wrapped with `tracedProxy` for automatic span creation
...
- **Auto-instrumentation** (SDK-level): HTTP server/client, Fastify route handlers, Pino log correlation (trace_id, span_id), Undici
- **Manual instrumentation — API**: `tracedProxy` wraps DI services; span name `ServiceName.methodName`, attribute `service.implementation`
- **Manual instrumentation — Game Generator**: `@traced` decorator on class methods, `withSpan` HOF on standalone functions
- **Shared core**: Both `@traced` and `withSpan` delegate to `executeInSpan` which handles sync/async, error recording, and span lifecycle
- **No-op when no SDK**: OpenTelemetry API returns no-op tracer when SDK is not registered
...
- **tracedProxy skips non-function properties**: Only method calls get spans; property access passes through
- **gameGenerator is NOT wrapped with tracedProxy**: It has internal `@traced` decorators instead (avoids double-spanning)
...
- **Tracing is opt-in**: `@opentelemetry/api` returns no-op when no SDK is registered; no runtime cost without a collector
- **`withSpan` passes span as first argument**: Wrapped functions receive `(span, ...originalArgs)`; callers of the wrapped function don't pass the span
...
- **Hint cells are prefilled and immutable**: Navigation skips them; input operations reject them
...
5. Add `withSpan` or `@traced` for tracing (if the function is on the hot path)
6. Add property-based tests alongside unit tests
...
1. Standalone function: wrap with `withSpan("spanName", (span, ...args) => { ... })`
2. Class method: add `@traced` decorator (span name auto-derived from `ClassName.methodName`)
3. DI service: wrap with `tracedProxy(target, "ServiceName")` in `singleton.ts`
4. All three approaches support sync and async, record exceptions automatically
