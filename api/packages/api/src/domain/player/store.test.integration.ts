...
describe("streak calculation", () => {
  ...
  it("AC6.1: current streak increments for consecutive calendar days with a solve", async () => {
    // Use dates ending today (2026-02-16) to ensure streak is current
    const today = new Date("2026-02-16T12:00:00Z");
    const yesterday = new Date("2026-02-15T12:00:00Z");
    const dayBefore = new Date("2026-02-14T12:00:00Z");
  ...
  it("AC6.2: best streak reflects the longest consecutive run in history", async () => {
    ...
    const recentRun = [new Date("2026-02-15T12:00:00Z"), new Date("2026-02-16T12:00:00Z")];
  ...
  it("AC6.4: multiple solves on the same day count as 1 streak day", async () => {
    ...
    // Insert two sessions on the same day (today)
    await db.insert(gameSessions).values({
      playerId,
      gameId: "game-same-day-1",
      completionTime: 100,
      solvedAt: new Date("2026-02-16T09:00:00Z"),
    });
    await db.insert(gameSessions).values({
      playerId,
      gameId: "game-same-day-2",
      completionTime: 110,
      solvedAt: new Date("2026-02-16T15:00:00Z"),
    });
